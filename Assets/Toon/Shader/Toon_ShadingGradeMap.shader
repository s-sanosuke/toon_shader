...
////// Lighting:
                ...
                float3 Set_RimLight = lerp( _LightDirection_MaskOn_var, (_LightDirection_MaskOn_var+(lerp( _Ap_RimLightColor.rgb, (_Ap_RimLightColor.rgb*Set_LightColor), _Is_LightColor_Ap_RimLight )*saturate((lerp( (node_3793 + ( (node_4691 - _RimLight_InsideMask) * (node_5985 - node_3793) ) / (node_5985 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_4691), _Ap_RimLight_FeatherOff )-(saturate(node_9044)+_Tweak_LightDirection_MaskLevel))))), _Add_Antipodean_RimLight );
                ...
                float3 node_8174 = (DecodeLightProbe( normalDirection )*_GI_Intensity);
                float3 Set_FinalCompOut = saturate((1.0-(1.0-node_4298)*(1.0-node_8174))); // Final Composition
